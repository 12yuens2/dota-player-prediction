...
The dataset used in this experiment contained 58 players. 123 replays used were from a single player as the positive sample, while 2-3 replays were taken from each of the other 57 players, giving a total of 307 replays parsed with 123 positive samples and 184 negative samples.
...
\centering
\begin{tikzpicture}
\newaxis{Accuracy of mouse movement actions}{Accuracy rate}{0.6}{0.85}{9cm}{5cm}{(0.5,-0.35)}
...
\newaxis{Precision of mouse movement actions}{Precision rate}{0.6}{0.85}{8cm}{6cm}{(0.5,-0.25)}
...
\begin{subfigure}{0.45\textwidth}
\begin{tikzpicture}[scale=0.8]
\newaxis{Recall of mouse movement actions}{Recall rate}{0.6}{0.85}{8cm}{6cm}{(0.5,-0.25)}
...
\begin{tabular}{| c | c |}
\hline
\makecell{True positives \\ 10} & \makecell{False positives \\ 0} \\ \hline
\makecell{False negatives \\ 26} & \makecell{True negatives \\ 29} \\ \hline
\end{tabular}
\caption{Confusion matrix of prediction samples from 5-fold cross validation for the random forest using only mouse movement features.}
...
Logistic regression continued to perform well when the features are combined, but few are better compared to using each featureset individually. Using game statistics and boots only encoding allowed the logistic regression model to achieve 99\% accuracy, which is odd as the boots only encoding had the lowest accuracy of all item encoding methods for logistic regression. Furthermore, using mouse movement, game statistics and one-hot encoding of all items led to a drastic reduction in accuracy in an odd anomaly.
...
There seems to be two different reasons for the varied performance and the overall very strong results of > 95\% accuracy for many features. First, the varied performance may come from the combination of features creating noise or interfering with each other, causing lowered accuracy compared to using some features individually. It may also be that the already strong performance makes the models overfit to the particular features, so the addition of more features that do not contribute well creates noise that affects performance drastically. Regarding the performance of the models being very strong, this may come from the problem itself being a relatively simple one as the models only have to determine the behaviour of one player out of a small pool. Overfitting to the particular dataset is also likely. Rudimentary tests on different players and heroes continue to show similar performance, but it is difficult to tell without more rigorous work on additional datasets. 
...
\begin{tikzpicture}
\begin{axis}[
title={\textbf{Difference in accuracy between itemisation features and itemisation features combined with mouse movement features}},
title style={align=center,text width={11cm}},
bar width=1em,
...
extra y ticks=0,
extra y tick labels={},
extra y tick style={grid=major,major grid style={dashed,draw=black}}
]

\diffbarplot{-0.01}{-0.242}{-0.0205}{Items hashed}{-.75cm}
\resetstackedplots
\diffbarplot{-0.021}{-0.272}{0.0399}{Items onehot}{-.25cm}
\resetstackedplots
\diffbarplot{0.0194}{-0.30263}{-0.02052}{Items starting}{.25cm}
\resetstackedplots
\diffbarplot{0.04947}{-0.002}{0.0394}{Items boots}{.75cm}
\resetstackedplots
\end{axis}
\end{tikzpicture}
